function fitTitulo() {
    const el = document.getElementById("titulo");
    if (!el) return;
    if (window.innerWidth >= 900) {
        el.style.fontSize = "3.5rem";
        return;
    }
    el.style.fontSize = "100px";
    const header = document.querySelector("header");
    const headerStyle = window.getComputedStyle(header);
    const marginLeft = parseInt(headerStyle.paddingLeft) || 40;
    const margin = marginLeft * 2;
    const scale = (window.innerWidth - margin) / el.scrollWidth;
    el.style.fontSize = (100 * scale) + "px";
}

function fitHero() {
    const el = document.getElementById("hero");
    if (!el) return;
    el.style.fontSize = "100px";
    const scale = (window.innerWidth * 1.5) / el.scrollWidth;
    el.style.fontSize = (100 * scale) + "px";
}

function fitTextTitle() {
    const el = document.getElementById('text-title');
    if (!el) return;
    const base = window.innerWidth <= 600 ? 1.1 : 1.8;
    el.style.fontSize = base + 'rem';
    const available = el.parentElement.offsetWidth;
    if (el.scrollWidth > available) {
        const scale = available / el.scrollWidth;
        el.style.fontSize = Math.max(0.6, base * scale) + 'rem';
    }
}

function positionContentArea() {
    const nav = document.getElementById('nav-menu');
    if (!nav) return;
    const lastLink = nav.querySelector('a:last-child');
    if (!lastLink) return;
    const contentArea = document.getElementById('content-area');
    if (!contentArea) return;
    const offset = window.innerWidth <= 600 ? 10 : window.innerWidth >= 900 ? -15 : 50;
    contentArea.style.top = (lastLink.getBoundingClientRect().bottom + offset) + 'px';
}

function positionFade() {
    const scrollArea = document.querySelector('.scroll-area');
    const fade = document.getElementById('content-area-fade');
    if (!scrollArea || !fade) return;
    fade.style.top = (scrollArea.getBoundingClientRect().bottom - 80) + 'px';
}

function refreshLayout() {
    fitTitulo();
    fitHero();
    positionContentArea();
    fitTextTitle();
    positionFade();
}

function injectMenu(pageKey) {
    const nav = document.getElementById('nav-menu');
    if (!nav) return;

    // Detecta se está numa subpasta (contos/, ensaios/, cronicas/)
    const partes = window.location.pathname.split('/').filter(Boolean);
    const emSubpasta = ['contos', 'ensaios', 'cronicas'].includes(partes[partes.length - 2]);
    const prefixo = emSubpasta ? '../' : '';

    const page = partes[partes.length - 1] || 'index.html';

    const links = [
        { id: 'contos',   href: prefixo + 'contos_index.html',   label: 'contos'    },
        { id: 'ensaios',  href: prefixo + 'ensaios_index.html',  label: 'ensaios'   },
        { id: 'cronicas', href: prefixo + 'cronicas_index.html', label: 'crônicas'  },
        { id: 'notas',    href: prefixo + 'anotacoes.html',       label: 'anotações' },
        { id: 'inicio',   href: prefixo + 'index.html',           label: 'início'    }
    ];

    const ehPaginaDeTexto = (
        page.startsWith('conto-')   ||
        page.startsWith('ensaio-')  ||
        page.startsWith('cronica-')
    );

    let html = '';
    links.forEach(item => {
        let texto = item.label;
        let classes = '';
        if (pageKey && item.id === pageKey) {
            classes = 'active';
            if (ehPaginaDeTexto) {
                if (item.id === 'contos')   texto = 'índice de contos';
                if (item.id === 'ensaios')  texto = 'índice de ensaios';
                if (item.id === 'cronicas') texto = 'índice de crônicas';
            }
        }
        html += `<a href="${item.href}" class="${classes}">${texto}</a>\n`;
    });
    nav.innerHTML = html;
}

function enableKeyboardScroll() {
    const scroller = document.getElementById('scroller');
    if (!scroller) return;
    const STEP = 120;
    document.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowDown' || e.key === 'PageDown') {
            e.preventDefault();
            scroller.scrollBy({ top: STEP, behavior: 'smooth' });
        }
        if (e.key === 'ArrowUp' || e.key === 'PageUp') {
            e.preventDefault();
            scroller.scrollBy({ top: -STEP, behavior: 'smooth' });
        }
    });
}

const SVG_EXPANDIR = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M3 8V5a2 2 0 0 1 2-2h3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
  <path d="M16 3h3a2 2 0 0 1 2 2v3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
  <path d="M21 16v3a2 2 0 0 1-2 2h-3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
  <path d="M8 21H5a2 2 0 0 1-2-2v-3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
</svg>`;

const SVG_RESTAURAR = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M8 3V5a2 2 0 0 1-2 2H3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
  <path d="M21 8h-3a2 2 0 0 1-2-2V3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
  <path d="M16 21v-2a2 2 0 0 1 2-2h3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
  <path d="M3 16h3a2 2 0 0 1 2 2v3" stroke="#fff" stroke-width="2.5" stroke-linecap="round"/>
</svg>`;

function injectBtnLeitura() {
    const scrollArea = document.querySelector('.scroll-area');
    const scrollInner = document.getElementById('scroller');
    if (!scrollArea || !scrollInner) return;

    const btn = document.createElement('button');
    btn.id = 'btn-leitura';
    btn.title = 'Modo leitura';
    btn.innerHTML = SVG_EXPANDIR;

    btn.addEventListener('click', () => {
        const ativo = document.body.classList.toggle('leitura');
        btn.innerHTML = ativo ? SVG_RESTAURAR : SVG_EXPANDIR;
        btn.title = ativo ? 'Restaurar' : 'Modo leitura';
        refreshLayout();
    });

    scrollArea.appendChild(btn);
}

function injectModal() {
    const div = document.createElement('div');
    div.id = 'modal-overlay';
    div.innerHTML = `
        <div id="modal-box">
            <div id="modal-titulo">Luiz Fonte Boa: ficção breve<button id="modal-fechar">✕</button></div>
            <div id="modal-corpo">
                Contos, ensaios e crônicas de autoria de Luiz&nbsp;Fonte&nbsp;Boa. Alguns textos contêm linguagem que pode vir a ser considerada imprópria para público jovem.<br>Qualquer semelhança com eventos ou pessoas reais<br>é mera coincidência<br>Ficção literária.
                <button id="modal-contato">Entrar em contato</button>
                <div id="modal-email"></div>
            </div>
        </div>`;
    document.body.appendChild(div);

    document.querySelector('nav').addEventListener('click', function(e) {
        if (e.target.closest('a')) return;
        div.classList.add('visivel');
    });
    div.addEventListener('click', function(e) {
        if (e.target === this) fecharModal();
    });
    document.getElementById('modal-fechar').addEventListener('click', fecharModal);
    document.getElementById('modal-contato').addEventListener('click', function() {
        const el = document.getElementById('modal-email');
        el.textContent = 'fonteboa' + '@' + 'gmail.com';
        el.style.display = 'block';
    });

    function fecharModal() {
        div.classList.remove('visivel');
        const el = document.getElementById('modal-email');
        el.style.display = 'none';
        el.textContent = '';
    }
}

function initLayout(pageKey) {
    injectMenu(pageKey);
    injectModal();
    document.fonts.ready.then(() => {
        refreshLayout();
        if (document.getElementById('scroller')) {
            enableKeyboardScroll();
            injectBtnLeitura();
        }
    });
    window.addEventListener("resize", refreshLayout);
}
